...
class Graph {                        // 图Graph模板类
 ...
 public:
  ...
  virtual int inDegree(int) = 0;   // 顶点v得入度(该顶点的确存在)
  virtual int outDegree(int) = 0;  // 顶点v得出度(该顶点的确存在)
  virtual int firstNbr(int) = 0;   // 顶点v的首个邻接顶点
  virtual int nextNbr(int, int) = 0;  // 顶点v得(相对于顶点j的)下一个邻接点
  virtual VStatus& status(int) = 0;  // 顶点v得状态
...
void Graph<Tv, Te>::DFS(int v, int& clock) {  // assert:0 <= v < n
  ...
  for (int u = firstNbr(v); -1 < u; u = nextNbr(v, u))  // 枚举v的所有邻居u
    switch (status(u)) {  // 并视其状态分别处理
      case UNDISCOVERED:  // u尚未发现,意味着支撑树可再次拓展
        type(v, u) = TREE;
        parent(u) = v;
        DFS(u, clock);
        break;
      case DISCOVERED:  // u已经被发现但尚未访问完毕,应属被后代指向的祖先
        type(v, u) = BACKWARD;
        break;
      default:  // u已经访问完毕(VISITED,有向图),应视承袭关系分为前向边或夸边
...
Stack<Tv>* Graph<Tv, Te>::tSort(int s) {  // assert:0 <= s < n
  ...
  return S;  // 若输入为DAG,则S内各项顶点自顶向低排序;否则(不存在拓扑排序),S空
}

template <typename Tv, typename Te>  // 基于DFS的拓扑排序算法(单趟)
bool Graph<Tv, Te>::TSort(int v, int& clock,
                          Stack<Tv>* S) {  // assert: 0 <= v < n
  dTime(V) = ++clock;
  status(v) = DISCOVERED;                               // 发现顶点v
  for (int u = firstNbr(v); -1 < u; u = nextNbr(v, u))  // 枚举v得所有邻居u
...
#define hca(x) (fTime(x))            // 利用此处限制的fTime[]充当hca[]
template <typename Tv, typename Te>  // 顶点类型,边类型
void Graph<Tv, Te>::BCC(int v, int& clock,
                        Stack<int>& S) {  // assert: 0 <= v < n
  // v被发现并入栈
  hca(v) = dTime(v) = ++clock;
  status(v) = DISCOVERED;
  S.push(v);
    ...
    switch (status(u)) {  // 并视u的状态分别处理
      case UNDISCOVERED:
        ...
        if (hca(u) < dTime(v))  // 遍历返回后,若发现u(通过后向边)可指向v的真祖先
          ...
          S.push(v);  // 最后一个顶点(关节点)从新入栈——分摊不足一次
        }
        break;
      case DISCOVERED:
        ...
        break;
      default:  // VIVSITED(digraphs only)
        type(v, u) = (dTime(v) < dTime(u)) ? FORWARD : CROSS;
        break;
    }
  status(v) = VISITED;  // 对v得访问结束
}
#undef hca

template <typename Tv, typename Te>  // 优先级搜索(全图)
template <typename PU>
...
void Graph<Tv, Te>::PFS(int s, PU prioUpdater) {  // 优先级搜索(单个连通域)
  ...
  while (1) {  // 将下一顶点和边加至PFS树中
    ...
    for (int shortest = INT_MAX, w = 0; w < n; w++)
      if (UNDISCOVERED == status(w))   // 尚未加入遍历树的顶点中
...
struct DijkstraPU {  // 针对Dijkstra算法的顶点优先级更新器
  virtual void operator()(Graph(Tv, Te) * g, int uk, int v) {
    if (UNDISCOVERED ==
        g->status(v))  // 对于uk每一尚未被发现的邻接顶点v,按Dijkstra策略
      if (g->priority(v) > g->priority(uk) + g->weight(uk, v)) {  // 做松弛
        g->priority(v) = g->priority(uk) + g->weight(uk, u);  // 更新优先级(数)
