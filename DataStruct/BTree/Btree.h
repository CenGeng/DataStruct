...
bool BTree<T>::insert(const T& e) {  // 将关键码e插入B树中
  ...
  if (v) return false;
  Rank r =
      _hot->key.search(e);  // 在节点_hot的有序关键码向量中查找合适的插入位置
...
void BTree<T>::solveOverflow(BTNodePosi(T) v) {
  if (_order >= v->child.size()) return;  // 递归基:当前接待你并未上溢
  Rank s = _order / 2;  // 轴点(此时应有_order = key.size() = child.size() - 1)
  BTNodePosi(T) u = new BTNode<T>();  // 注意:新节点已有一个空孩子
  for (Rank j = 0; j < _order - s - 1;
       j++) {  // v右侧_order-s-1个孩子及关键码分裂为右侧节点u
  ...
  Rank r = 1 + p->key.search(v->key[0]);  // p中指向u得指针的秩
  p->key.insert(r, v->key.remove(s));     // 轴点关键码上升
  // 新节点u与父节点p互联
  p->child.insert(r + 1, u);
  u->parent = p;
  solveOverflow(p);  // 上升一层,如有必要则继续分裂——至多递归O(logn层)
}
...
void BTree<T>::solveUnderflow(BTNodePosi(T) v) {
   ...
   *另外,在实现了孩子指针的判等器之后,也可以直接调用Vectir::find()定位
   */
  // 情况1:向左兄弟借关键码
  if (0 < r) {                           // 若v不是p的第一个孩子,则
    ...
    if ((_order + 1) / 2 < ls->child.size()) {  // 若该兄弟足够“胖”,则
      ...
      p->key[r - 1] =
          ls->key.remove(ls->key.size() - 1);  // ls的最大关键码转入p
  ...
  if (p->child.size() - 1 > r) {         // 若v不是p的最后一个孩子,则
    ...
    if ((_order + 1) / 2 < rs->child.size()) {  // 若该兄弟足够“胖”,则
      ...
      return;  // 至此,通过左旋已完成当前层(以及所有层的下溢处理)
    }
  }  // 至此,右兄弟要么为空,要么太“瘦”
  ...
  if (0 < r) {                           // 与左兄弟合并
    ...
    if (ls->child[ls->child.size() - 1])  // v的最左侧孩子过继给ls左最右侧孩子
    ...
    while (!v->key.empty()) {  // v剩余的关键码和孩子,一次转入rs
  ...
  solveUnderflow(p);  // 上升一层,如有必要则继续分裂——至多递归O(logn层)
  return;
}